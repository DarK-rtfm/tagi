...
namespace tools {
template <typename T> class subnetgen {
private:
  IP::NET<T> pool;
...
public:
  subnetgen(IP::NET<T> _pool, const std::vector<IP::MASK<T>> &_masks);
  subnetgen(IP::NET<T> _pool, const std::vector<T> &_reqs);
  int validate();
  std::vector<IP::NET<T>> generate();
};

template <typename T> std::vector<IP::NET<T>> subnetgen<T>::generate() {
